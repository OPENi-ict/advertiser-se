    ...
    var openi_req = https.request({
        ...
        function (openi_res) {
			...
			try{
                ...
                var demographicsJSON = demographics(search_res_json, req.body.demographics);
...
function decodeReq(reqBody) {
    ...
    try {
        ...
        var audMngArray = req_json.audMng;

        var jsonToQuery = "";
        for (var obj in audMngArray) {
            if (jsonToQuery === "")
...
function demographics(contextObjs, reqAttr) {
    ...
    for (var demographAttrName in test) {      // attribute name in request                                                             // list with available/requested attributes of object
        for (var cntxtObjNum = 0; cntxtObjNum < contextObjs.length; cntxtObjNum++) {    // search in every object from the cloudlet search response
            if (typeof test[demographAttrName] === "object"){
                for (var j=0; j<test[demographAttrName].length; j++){
                    ////////////////////////////////////////////////////////////////////////////////////////////////////
                    if (contextObjs[cntxtObjNum]['@data'].hasOwnProperty(demographAttrName)) {    //  check if cloudlet context #[cntxtObjNum] has the search property from client req               /*innerObj == cntx*/
                        var cntxAttrVal = contextObjs[cntxtObjNum]['@data'][demographAttrName];  // the value of the cloudlet context property
